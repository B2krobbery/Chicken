...
- **Authentication & Registration**: Dual-mode login and registration frontend now live on `/`. Supports commercial Buyer (Hotels, Restaurants, QSRs, Caterers) and Supplier self-onboarding with immediate transition to pending KYC workflows. Production-grade Google OAuth SSO is integrated across frontend and backend (`POST /api/v1/auth/google`), supporting Google One Tap / Sign In, auto-provisioning, and JWT issuance. Reactive 401 session expiry clears stale tokens.
