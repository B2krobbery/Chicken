  ...
  const sidebar = (
    <div className="flex flex-col h-full">
      ...
      <div className="h-12 flex items-center gap-2.5 px-4 border-b border-shell-700 shrink-0">
        <div className="w-7 h-7 rounded-md bg-gradient-to-tr from-brand-600 to-amber-500 flex items-center justify-center text-sm font-black text-white shrink-0">
          🍗
        </div>
        ...
        {role && (
          <span
            className={`ml-auto text-[9px] font-bold px-1.5 py-0.5 rounded border uppercase tracking-wider shrink-0 ${
        ...
        {nav.map((item) => {
          ...
          return (
            <button
              ...
              className={`w-full flex items-center gap-2.5 px-2.5 py-2 rounded-md text-[13px] font-medium transition text-left ${
                active
            ...
            >
              ...
              {item.badge !== undefined && item.badge !== 0 && (
                <span className="text-[10px] font-bold bg-shell-700 text-slate-200 px-1.5 py-0.5 rounded min-w-[20px] text-center tnum">
                  {item.badge}
                </span>
              )}
            </button>
      ...
      {/* User footer */}
      <div className="border-t border-shell-700 p-3 space-y-1.5">
        <div className="flex items-center gap-2.5">
          <div className="w-7 h-7 rounded-full bg-shell-700 flex items-center justify-center text-[11px] font-bold text-slate-300 shrink-0">
            {(user?.full_name || "?")
              .split(" ")
              .map((w) => w[0])
              .slice(0, 2)
  ...
  return (
    <div className="min-h-screen flex bg-slate-50">
      ...
      <div className="flex-1 min-w-0 lg:ml-60 flex flex-col min-h-screen">
        {/* Top bar */}
        <header className="h-12 bg-white border-b border-slate-200 sticky top-0 z-30 flex items-center gap-3 px-3 sm:px-4">
          <button
          ...
          {/* Env chip */}
          <span className="hidden sm:inline-flex items-center gap-1 text-[10px] font-bold px-1.5 py-0.5 rounded border border-slate-200 bg-slate-50 text-slate-500 uppercase tracking-wide">
          ...
          >
            ...
            <span className="hidden md:block text-xs font-semibold text-slate-700 max-w-[140px] truncate">
              {user?.full_name}
            </span>
          </button>
          {userMenuOpen && (
            <div className="absolute right-3 top-11 w-52 bg-white border border-slate-200 rounded-md shadow-lg z-50 py-1 text-xs">
        ...
        {/* Content */}
        <main className="flex-1 p-3 sm:p-4 lg:p-5 min-w-0">{children}</main>
      </div>
    </div>
  );
}
...
function StatusBadgeMini({ status }: { status: string }) {
  ...
  return (
    <span
      className={`px-1.5 py-0.5 rounded border text-[10px] font-bold uppercase ${c}`}
    >
      {status}
    </span>
  );
